...
namespace BVA.Sample
{
    public class SampleScript : MonoBehaviour
    {
        ...
        public async void LoadBVA()
        {
            ...
            CpuWatch.Stop();
            modelType = ModelType.BVA;
            logText.text += $"成功加载BVA模型 : {paths[0]} Load Time :{CpuWatch.Ellapsed}s\n";
        }
        ...
        public void ExportAvatar()
        {
            ...
            var path = SFB.StandaloneFileBrowser.SaveFilePanel("Runtime Export", "", name, BVAConst.EXTENSION_BVA);
            if (!string.IsNullOrEmpty(path))
            {
                exporter.SaveGLB(Path.GetDirectoryName(path), Path.GetFileNameWithoutExtension(path), BVAConst.EXTENSION_BVA);
            }
