...
namespace GLTF
{
    public static class GLTFHelpers
    {
        private struct PreviousGLTFSizes
        {
            ...
            public int PreviousAnimatorClipCount;
        ...
        public static void MergeGLTF(GLTFRoot mergeToRoot, GLTFRoot mergeFromRoot)
        {
            PreviousGLTFSizes previousGLTFSize = new PreviousGLTFSizes()
            {
                ...
                PreviousAnimatorClipCount = mergeToRoot.Extensions.HumanoidAnimationClips?.Count ?? 0,
        ...
        public static uint LoadBufferView(BufferView bufferView, BufferCacheData bufferData, out byte[] bufferViewCache)
        {
            uint totalOffset = bufferView.ByteOffset + bufferData.ChunkOffset;
            var stream = bufferData.Stream;
        ...
        private static void MergeAnimationsAndSkins(GLTFRoot mergeToRoot, GLTFRoot mergeFromRoot, PreviousGLTFSizes previousGLTFSizes)
        {
            ...
            if (mergeFromRoot.Extensions.HumanoidAnimationClips != null)
            {
                if (mergeToRoot.Extensions.HumanoidAnimationClips == null)
                {
                    mergeToRoot.Extensions.HumanoidAnimationClips = new List<GLTFAnimation>(mergeFromRoot.Extensions.HumanoidAnimationClips.Count);
                }

                mergeToRoot.Extensions.HumanoidAnimationClips.AddRange(mergeFromRoot.Extensions.HumanoidAnimationClips);

                for (int i = previousGLTFSizes.PreviousAnimationCount; i < mergeToRoot.Extensions.HumanoidAnimationClips.Count; ++i)
                {
                    GLTFAnimation animation = mergeToRoot.Extensions.HumanoidAnimationClips[i];
        ...
        public static NodeId FindCommonAncestor(IEnumerable<NodeId> nodes)
        {
            ...
            int? commonAncestorIndex = nodes.Select(n => n.Id).Aggregate((int?)null, (elder, node) => FindCommonAncestor(elder, node));
