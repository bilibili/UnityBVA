...
namespace BVA
{
    public partial class GLTFSceneImporter
    {
            ...
            for (int i = 0; i < _gltfRoot.Textures.Count; i++)
            {
                ...
                if (_assetCache.ImageCache[sourceId] == null)
                {
                    ...
                    else
                    {
                        ...
                        unity_texture.LoadImage(buffer, Application.isPlaying && !Application.isEditor);
        ...
        {
            _unityMeshDatas = new UnityMeshData[_gltfRoot.Meshes.Count];
            List<ParallelLoopResult> parallelLoopResults = new List<ParallelLoopResult>();
            Parallel.For(0, _gltfRoot.Meshes.Count, (meshIndex) => /*for (int meshIndex = 0; meshIndex < _gltfRoot.Meshes.Count; meshIndex++)*/
            {
                ...
                if (targets != null)
                {
                    for (int j = 0; j < targets.Count; ++j)
                    {
                        ...
                        {
                            if (vertOffset == 0)
                                unityData.MorphTargetVertices[j] = targets[j][SemanticProperties.POSITION].AccessorContent.AsVec3s;
                            else
                                targets[j][SemanticProperties.POSITION].AccessorContent.AsVec3s.ToUnityRaw(unityData.MorphTargetVertices[j], vertOffset);
                        }
                        if (targets[j].ContainsKey(SemanticProperties.NORMAL))
                        {
                            if (vertOffset == 0)
                                unityData.MorphTargetNormals[j] = targets[j][SemanticProperties.NORMAL].AccessorContent.AsVec3s;
                            else
                                targets[j][SemanticProperties.NORMAL].AccessorContent.AsVec3s.ToUnityRaw(unityData.MorphTargetNormals[j], vertOffset);
                        }
                        if (targets[j].ContainsKey(SemanticProperties.TANGENT))
                        {
                            if (vertOffset == 0)
                                unityData.MorphTargetTangents[j] = targets[j][SemanticProperties.TANGENT].AccessorContent.AsVec3s;
                            else
                                targets[j][SemanticProperties.TANGENT].AccessorContent.AsVec3s.ToUnityRaw(unityData.MorphTargetTangents[j], vertOffset);
                        }
                    }
                }
                for (int i = 0; i < mesh.Primitives.Count; ++i)
                {
                    ...
                    submeshAttributes[SemanticProperties.INDICES].AccessorContent.AsUInts.ToIntArrayFlipTriangleFaces() : MeshPrimitive.GenerateIndices(vertexCount) :
                     submeshAttributes.ContainsKey(SemanticProperties.INDICES) ? submeshAttributes[SemanticProperties.INDICES].AccessorContent.AsUInts.ToIntArray() : MeshPrimitive.GenerateIndices(vertexCount);
                    ...
                    if (firstPrimContainsAll && i == 0 || !firstPrimContainsAll)
                    {
                        ...
                        {
                            if (vertOffset == 0)
                                unityData.Vertices = meshAttributes[SemanticProperties.POSITION].AccessorContent.AsVec3s;
                            else
                        ...
                        {
                            if (vertOffset == 0)
                                unityData.Normals = meshAttributes[SemanticProperties.NORMAL].AccessorContent.AsVec3s;
                            else
                        ...
                        {
                            if (vertOffset == 0)
                                unityData.Tangents = meshAttributes[SemanticProperties.TANGENT].AccessorContent.AsVec4s;
                            else
                        ...
                        {
                            if (vertOffset == 0)
                                unityData.Uv1 = meshAttributes[SemanticProperties.TexCoord[0]].AccessorContent.AsVec2s;
                            else
                ...
                _unityMeshDatas[meshIndex] = unityData;

                Statistics.VertexCount += vertOffset;
            });
        }
        private ParallelLoopResult LoadMeshTargetAttribute(Dictionary<string, AttributeAccessor> att)
        {
            return Parallel.ForEach(att, (kvp) =>
            {
        ...
        private ParallelLoopResult LoadMeshAttribute(Dictionary<string, AttributeAccessor> att)
        {
            return Parallel.ForEach(att, (kvp) =>
            {
                AttributeAccessor attributeAccessor = kvp.Value;
