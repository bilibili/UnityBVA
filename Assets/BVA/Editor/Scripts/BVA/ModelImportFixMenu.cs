...
namespace BVA
{
    ...
    public class ModelImportFixMenu : EditorWindow
    {
        ...
        static List<ConditionSolusionPair<ModelImporter>> modelSolutionReverts = new List<ConditionSolusionPair<ModelImporter>>()
       {
        ...
        static ModelImportFixMenu modelImportWindow;
        ...
        public static void Init()
        {
            ...
            {
                modelImportWindow = (ModelImportFixMenu)EditorWindow.GetWindow(typeof(ModelImportFixMenu), false, "Fix Import Model");
            }
        ...
        void CheckValidity()
        {
            ...
            bool isFixAllTextrue = isFixAll;
            if (GUILayout.Button("Fix AllTextrue"))
            {
                isFixAllTextrue = true;
                Undo.RecordObjects(textureImporterList.ToArray(), "Redo Texture");
            }
            if (isShowAllTexture|| isFixAllTextrue)
            {
                if (isRevert)
                {
                    ShowList(textureImporterList, textureSolutionReverts, isFixAllTextrue);
                }
                else
                {
                    ShowList(textureImporterList, textureSolutions, isFixAllTextrue);
                }
            ...
            bool isFixAllModel = isFixAll;
            if (GUILayout.Button("Fix All Model"))
            {
                isFixAllModel = true;
                Undo.RecordObjects(modelImporterList.ToArray(), "Redo Model");
            }
            if (isShowAllModel|| isFixAllModel)
            {
                if (isRevert)
                {
                    ShowList(modelImporterList, modelSolutionReverts, isFixAllModel);
                }
                else
                {
                    ShowList(modelImporterList, modelSolutions, isFixAllModel);
                }
        ...
        void ShowList<T>(List<T> targetImporters, List<ConditionSolusionPair<T>> conditionSolusionPairs,bool isFixAll) where T: AssetImporter
        {
            for (int i = 0; i < conditionSolusionPairs.Count; i++)
            {
                ...
                for (int ii = 0; ii < targetImporters.Count; ii++)
                {
                    ...
                    if (Condition(importer))
                    {
                        ...
                        if (GUILayout.Button("Fix it") || isFixAll)
                        {
