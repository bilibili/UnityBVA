	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		void GetTarget()
		{
			...
			if(nearAxis == Axis.None && Input.GetMouseButtonDown(0))
			{
				...
				//RaycastHit[] hitInfo = Physics.SphereCastAll(ray, 1f, Mathf.Infinity);
				if (Physics.SphereCast(ray, 0.1f, out RaycastHit hitInfo, Mathf.Infinity))
				{
					Transform target = hitInfo.transform;
					Animator animator = target.gameObject.GetComponentInParent<Animator>();
					if (animator != null) 
